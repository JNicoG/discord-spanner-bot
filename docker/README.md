...
- `init-scripts/` — optional SQL files copied into the image on first initialization (use with caution)
- Management scripts:
...
- The Docker image copies SQL files from `init-scripts/` into `/docker-entrypoint-initdb.d/` and those scripts are executed only when the DB data directory is empty. If you use `init-scripts/` and Flyway migrations together, make sure they don't conflict (see project root README for strategies).
